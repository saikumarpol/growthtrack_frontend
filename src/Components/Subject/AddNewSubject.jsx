...
const bucketName = "pmis-001";  
  ...
  const validateForm = async () => {
    ...
    // Only require audio/consent if not in edit mode
    if (!isEditMode) {
      if (!audioURL) {
        await swal({
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      // Only upload audio if not in edit mode
      let audioFileLocation = null;
      if (!isEditMode) {
        const recordingName = `${name}_${phoneNumber}_${Date.now()}.wav`;
        audioFileLocation = await uploadToAWS(recordingName, audioURL);
      ...
      // Only append consent if not in edit mode
      if (!isEditMode) {
          ...
          <div className="card">
            ...
            <div className="card-body">
              <form onSubmit={handleSubmit}>
                ...
                {/* Consent/Audio section: only show if not in edit mode */}
                {!isEditMode ? (
                  <div className="mb-4">
                    <div
                      style={{
                  ...
                  <div className="mb-4">
                    ...
                    >
                      <span>{t("Consent already provided for this subject.")}</span>
                    </div>
                  </div>
                )}
